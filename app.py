...
def predict():
    ...
    return render_template("yield_prediction.html", prediction_text=pred, yield_predictions=float(yields))
...
@app.route("/dis_pre", methods=["POST"])
def dis_pre():
    if request.method == "POST":
        print(request.form)
        ...
        pred_args = [District_Name, Crop, Area, Soil_type]
        pred_args_arr = np.array(pred_args)
        pred_args_arr = pred_args_arr.reshape(1, -1)
        output = model.predict(pred_args_arr)
        print(output)
    ...
    return render_template("districts.html", prediction_text=pred, yield_predictions=float(yields))
